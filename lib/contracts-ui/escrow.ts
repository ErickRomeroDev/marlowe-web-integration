...
const projectTag: Tags = { MARLOWE_ESCROW1: {} };
export const tags_array = ["MARLOWE_ESCROW1"];
...
export function projectGetStatePlus(state: ProjectState, scheme: ProjectParameters) {
  switch (state.type) {
    case "InitialState":
      ...
      return { printResult: `Awaiting role payer's deposit of ${(Number(scheme.amount) / 1000000).toFixed(2)} ADA.` };
    case "PaymentDeposited":
      ...
      return { printResult: `Funds deposited. Awaiting release on ${new Date(scheme.releaseDeadline).getMonth() + 1}/
...
export async function applyInputChoice(contractInstance: ContractInstanceAPI, value: CanChoose): Promise<TxId> {
  const applicableActions = await contractInstance.evaluateApplicableActions();
