...
export const tags_array = ["PAYMENT_SPLITTER"];
...
export type ProjectActions = Array<{
  name: string;
  description?: string;
  value: CanDeposit | CanAdvance | CanChoose | null;
}>;
...
const address5 = addressBech32(
  "addr_test1qr36c9ccg8j6e4qda5tk0e97j6yxgh3txrufz79r8y4nutjwladqsndw3y6r3t5ra7ecys6uplm0glyx24kvfm9t5x8slvtwst"
);
const address6 = addressBech32(
  "addr_test1qzwkamtf40cxr9tth7fz087e8f8ynr5vr7why3nw8r0d4r2wladqsndw3y6r3t5ra7ecys6uplm0glyx24kvfm9t5x8svmp6cn"
);
const address7 = addressBech32(
  "addr_test1qp6ypxq3wsgy69cz0qkmcmdfrr330rv7hhcxl9pla2gj6c2wladqsndw3y6r3t5ra7ecys6uplm0glyx24kvfm9t5x8sqm8fh6"
);

function mkBundle(
  scheme: ProjectParameters
): ContractBundleMap<ProjectAnnotations> {
  return {
...
export async function projectGetState(
  ...
  switch (txOut.contract.annotation) {
    ...
    case "PaymentCancelClose":
      return { type: "Closed", result: "Payment canceled" };
  }
}

export function projectGetStatePlus(
  state: ProjectState,
  scheme: ProjectParameters
) {
  switch (state.type) {
    case "InitialState":
      ...
      return {
        printResult: `Awaiting role payer's deposit of ${(Number(scheme.amount) / 1000000).toFixed(2)} ADA.`,
      };
    case "PaymentDeposited":
      ...
      return {
        printResult: `Funds deposited. Awaiting release on ${new Date(scheme.releaseDeadline).getMonth() + 1}/
      ${new Date(scheme.releaseDeadline).getDate()}/
      
      ${new Date(scheme.releaseDeadline).getFullYear().toString().slice(-2)}
      ${""}
      ${new Date(scheme.releaseDeadline).getHours()}:
      ${new Date(scheme.releaseDeadline)
        .getMinutes()
        .toString()
        .padStart(2, "0")}.`,
      };
    case "PaymentMissed":
      ...
      return {
        printResult: `Deposit period expired. Close contract to retrieve min eUTxO.`,
      };
    case "PaymentReady":
      console.log(`Payment ready to be released`);
      return { printResult: `Funds ready for release!` };
    case "Closed":
...
export async function projectGetMyActions(
  contractInstance: ContractInstanceAPI,
  state: ProjectState
): Promise<ProjectActions> {
...
export async function projectGetActions(
  contractInstance: ContractInstanceAPI,
  state: ProjectState
): Promise<ProjectActions> {
  const applicableAction = await contractInstance.evaluateApplicableActions();
  return [
    ...applicableAction.actions.map((action) => {
      switch (action.type) {
        case "Deposit":
          return {
            name: `Deposit ${(Number(action.deposit.deposits) / 1000000).toFixed(2)} ADA`,
            value: action,
          };
        default:
          throw new Error("Unexpected action type");
...
export async function applyInputDeposit(
  contractInstance: ContractInstanceAPI,
...
export async function applyInputChoice(
  contractInstance: ContractInstanceAPI,
  value: CanChoose
): Promise<TxId> {
  const applicableActions = await contractInstance.evaluateApplicableActions();
...
export async function existContractId(
  contractId: string,
  runtimeLifecycle: RuntimeLifecycle
) {
...
export async function getContractInfoPlus(
  id: string,
  runtimeLifecycle: RuntimeLifecycle
): Promise<ContractInfoPlus> {
  ...
  const details = await result.contractInstance.getDetails();
  if (details.type === "closed") {
    hasOpenRole = false;
  }
  const history = await result.contractInstance.getInputHistory();
  const applicableActions =
    await runtimeLifecycle.applicableActions.getApplicableActions(details);
  const depositAvailable = applicableActions.some(
...
export async function getContractsByAddress(
  ...
  const contractHeaders =
    await runtimeLifecycle.restClient.getContracts(contractsRequest);
...
export async function getContractsByToken(
  ...
  const filteredByRoleTokenMintingPolicy = contractHeaders.contracts.filter(
    (header) => header.roleTokenMintingPolicyId
  );
  ...
  const filteredByWalletTokens = (header: ContractHeader): boolean => {
    return walletTokens.some(
      (item) =>
        ...
        item.assetId.assetName === tokenAssetName
    );
  };

  //filter by tokens on the wallet
  const contractHeaderFilteredByWallet =
    filteredByRoleTokenMintingPolicy.filter((header) =>
      filteredByWalletTokens(header)
    );
...
export async function getContractsByOpenRole(
  ...
  //predicate
  const filteredByOpenRole = async (
    header: ContractHeader
  ): Promise<boolean> => {
    const contractInstance = await runtimeLifecycle.newContractAPI.load(
      header.contractId
    );
