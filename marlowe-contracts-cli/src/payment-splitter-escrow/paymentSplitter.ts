...
import { ContractId, Tags, addressBech32, lovelaces } from "@marlowe.io/runtime-core";
...
export const projectTag: Tags = { MARLOWE_ESCROW1: {} };
...
export type ProjectActions = Array<{
  name: string;
  description?: string;
  value: CanDeposit | CanAdvance | CanChoose | { type: "check-state" } | { type: "return" };
}>;
...
//use when wallet API
export async function projectValidation(lifecycle: RuntimeLifecycle, contractId: ContractId): Promise<ProjectValidationResults> {
  // First we try to fetch the contract details and the required tags
...
export function projectGetOpenRoleActions(applicableAction: NewApplicableActionsAPI, contractState: ProjectState): ProjectActions {
  return [
    {
...
//use when wallet API only (no option for wallet address)
export function projectGetMyActions(applicableAction: NewApplicableActionsAPI, contractState: ProjectState): ProjectActions {
  return [
    {
      name: "Re-check contract state",
      value: { type: "check-state" },
...
export async function projectValidationRest(restClient: RestClient, contractId: ContractId): Promise<ProjectValidationResultsRest> {
  // First we try to fetch the contract details and the required tags
  const contractDetails = await restClient.getContractById({
    contractId,
  });
