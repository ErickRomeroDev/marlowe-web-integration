...
export const CreateContract = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    deploy(amount, values.address, values.name, values.githubUrl, values.date);
    console.log({ values });
  };
  ...
  const deploy = async (
    ...
    const { mkSourceMap } = await import(
      "@/lib/contracts-ui/experimental-features/source-map"
    );
    const { fundMyProjectTag, fundMyProjectTemplate, mkFundMyProject } =
    ...
    if (walletAddress && runtimeLifecycle) {
      try {
        ...
        form.reset();
        console.log(
          `Contract id ${contractInstance.id} was successfully submited to the blockchain`
        );
      } catch (error) {
        setLoading(false);
